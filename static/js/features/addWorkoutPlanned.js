export function initAddWorkout(root) {
  const container = root.querySelector('[data-feature="add-workout-planned"]')
  if (!container) return
  const jsonDataElement = root.querySelector('#addworkout-data')
  if (!jsonDataElement) return
  const { planNames } = JSON.parse(jsonDataElement.textContent)
  const planNames_justName = planNames.map(p => p.Name)
  ...
  button.addEventListener("click", () => {
    createWorkoutInput(container, planNames_justName)
  })
...
function createWorkoutInput(container, planNames) {
  ...
  for (let i = 0; i < planNames.length; i++) {
    ...
    option.value = planNames[i];
    option.textContent = planNames[i];
